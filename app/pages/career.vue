...
import type { LocalizedString, PortfolioCareerStep } from '../../shared/types';
...
function isLatestPositionForCompany(index: number): boolean {
  ...
  for (let i = 0; i < index; i += 1) {
    const previousStep = steps.value[i];

    if (!previousStep) {
      continue;
    }

    const previousCompanyKey = getCompanyKey(previousStep.company);
