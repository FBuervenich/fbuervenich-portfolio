  ...
  <Layout>
    <div class="hero container-inner mx-auto flex flex-col sm:flex-row justify-between py-16 mt-8">
      ...
      <div class="mt-8 sm:mt-0 w-1/5">
        <g-image src="/developer_activity.svg" alt="hero" class="mx-auto sm:mx-0" />
      </div>
    </div>

    <div class="container-inner mx-auto py-6">
      <i18n-t keypath="pages.index.introduction" class="text-lg sm:text-xl" tag="p">
        <template #nextaudit>
          <a href="https://next-audit.de/" target="_blank" rel="noopener">nextAudit</a>
        </template>
      </i18n-t>
      ...
      >
        ...
        <ul class="text-lg sm:text-xl space-y-6">
          <li v-for="step in steps" :key="String(step.id)" class="checkmark">
            ...
            <div class="text-lg text-gray-600 pre">{{ $translate(step.description) }}</div>
            <div class="text-lg text-gray-gray-900">
              {{ formatTechnologies(step.technologies) }}
            </div>
            <div v-if="step.projectUrl">
              <a class="inline-block" :href="String(step.projectUrl)" target="_blank" rel="noopener">
    ...
    <div class="overflow-x-hidden border-gray-200 border-b">
      ...
      <div class="get-to-know-me container-inner mx-auto text-xl pb-16 relative">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mb-16">
          <div>
            <g-image src="/Florentin.png" alt="avatar" class="w-32 h-32 rounded-full mb-8 lg:mb-0" />
          </div>
          <div class="flex-1 text-lg sm:text-xl ml-6">
            <div class="font-bold">{{ `${personalData.firstName || ''} ${personalData.lastName || ''}`.trim() }}</div>
            <div>
...
const steps = computed<ProjectStep[]>(() => {
  const projects = (data.value.projects as ProjectStep[]) || [];
...
function formatTechnologies(technologies?: LocalizedString[]): string {
  return technologies ? technologies.map((item) => useNuxtApp().$translate(item)).join(' | ') : '';
}
</script>
