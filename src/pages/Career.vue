      ...
      <div class="flex">
        <div class="md:w-4/5 w-full">
          <ol class="relative border-l border-gray-200 dark:border-gray-700">
            <li v-for="(step, index) in steps" :key="String(step.id)" class="mb-10 ml-6">
              <span
              ...
              <time class="block mb-2 text-sm font-normal leading-none text-gray-600 dark:text-gray-500">
                {{ getDurationString(step) }}
              </time>
              <div class="mb-4 text-base font-normal text-gray-800 dark:text-gray-400">
...
const steps = computed<CareerStep[]>(() => {
  const careerSteps = (data.value.career as CareerStep[]) || [];
...
function getDurationString(stepNode: CareerStep): string {
  ...
  const endDate = Number.isNaN(Date.parse(stepNode.endDate)) ? new Date() : new Date(stepNode.endDate);
  const months = monthDiff(startDate, endDate);

  return `${formatPeriod(startDate, stepNode.endDate)} · ${formatDuration(months)}`;
}
